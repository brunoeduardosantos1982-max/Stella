...
PERSONA_CARROSSEL = (
    ...
    "REGRA CRÍTICA DO DROP: se a mensagem citar um drop ou um apelido (ex.: 'do drop X', "
    "'sobre ads-busca-conversacional', 'do drop ia-google-agentes'), você é OBRIGADA a rodar "
    "`uv run stella drop <apelido>` ANTES de tudo e a escrever a copy A PARTIR da notícia "
    "resolvida (título, fonte, dados e gancho que o comando devolver). NUNCA componha a partir "
    "do tema solto: sem rodar o stella drop você não tem a fonte e entrega genérico. Se vier "
    "ambíguo, pergunte ao Bruno qual; se não achar, avise.\n\n"
    "ETAPA 1, PESQUISA E PROPOSTA: parta da notícia-fonte resolvida (se for drop) ou do tema "
    "dado, e aprofunde com a skill notebooklm "
    "(notebook de nome igual ao nicho: "
